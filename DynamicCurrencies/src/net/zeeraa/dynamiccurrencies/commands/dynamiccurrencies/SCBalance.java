...
public class SCBalance extends ZSubCommand {
	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args) {
		if (args.length == 0) {
			if (sender instanceof Player) {
				showInfo(sender, (OfflinePlayer) sender, false);
				return true;
			} else {
				sender.sendMessage(ChatColor.RED + "Please provide a player");
			}
		} else {
			if (sender.hasPermission("dynamiccurrencies.command.dynamiccurrencies.balance.others")) {
				OfflinePlayer player = Bukkit.getServer().getPlayer(args[1]);
				if (player != null) {
					showInfo(sender, player, false);
				} else {
	...
	private void showInfo(CommandSender sender, OfflinePlayer player, boolean isSelf) {
		if (DynamicCurrenciesAPI.hasAccount(player.getUniqueId())) {
			...
			sender.sendMessage(ChatColor.RED + (isSelf ? "You" : "That player") + " does not have an account");
		}
	}
